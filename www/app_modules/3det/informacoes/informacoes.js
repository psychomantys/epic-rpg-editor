                            ...
                            function($scope, CharacterService) {
                                ...
                                for (i = 0; i < $scope.character.strength.modifiers.length; ++i) {
                                    if ($scope.character.strength.modifiers[i].active)
                                        $scope.character.strength.total = CharacterService.addModifier($scope.character.strength.total, $scope.character.strength.modifiers[i]);
                                }

                                $scope.addStrModifier = function(modifier) {
                                    $scope.character.strength.total = CharacterService.addModifier($scope.character.strength.total, modifier);
                                }
                            }]);
